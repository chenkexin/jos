...
//	cprintf("in sys_env_set_status:%d\n", ret);
	if(ret) return ret;
	cprintf("setting the status\n");
	e->env_status = status;
	return 0;	
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if(p)
	{
		...
		int ret2 = page_insert(e->env_pgdir, p, va, perm);
		if(ret2)
		{
		 	 page_free(p);
			 return ret2;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if( ret ) return ret;
	int ret2 = envid2env(dstenvid, &de, 1);
	if( ret ) return ret2;

	pte_t *p;
	struct PageInfo* page = page_lookup( se->env_pgdir, srcva, &p );

	if( srcva >= (void*)UTOP|| dstva >= (void*)UTOP ) return -E_INVAL;
	//check align
  ...
  switch (syscallno) {
        ...
        case SYS_exofork:
				...
				case SYS_env_set_status:
						cprintf("case SYS_env_set_status\n");
